...
class BPCollector():
    ...
    def __init__(self, main_page, output_file, project="openstack", extended=False, size=100, with_def_milestone=True):
        self.p = main_page
    ...
    def update_prog(self, amount=1):
        ...
        self.p.pbar.setValue(self.p_value)
        #app.processEvents()
    ...
    def collect(self):
        ...
        while True:
            ...
            self.update_prog(min((int(content.get('total_size', 0)) - int(content.get('start', 0))), self.size))
...
class MainWindow(QtGui.QDialog):
    ...
    def collect_handler(self):
        ...
        try:
            BPCollector(self, self.output_file, self.project, extended=extended,
                        with_def_milestone=(not self.include_without_milestone.isChecked())).collect()
